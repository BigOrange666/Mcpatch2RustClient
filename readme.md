### McPatch2

McPatch第二版的Rust版客户端源代码。

### crates说明

| 名称                   | 用途                                                         |
| ---------------------- | ------------------------------------------------------------ |
| client         | 客户端主程序。用来执行文件更新过程，需要配置好后分发给玩家   |
| config_template_derive | 给客户端用的过程宏，用来根据源代码注释自动化生成客户端配置文件模板 |
| xtask                  | 用于ci/cd自动化打包的行为和命令                              |

### 常用命令说明

| 命令                                | 作用                                 |
| ----------------------------------- | ------------------------------------ |
| `cargo dev`                          | 开发场景下，启动客户端程序进行测试           |
| `cargo ci`                           | 自动构建场景下，打包客户端 |

添加gitee支持（由于gitee仓库限制，大更新无法使用）

v2.2.2
添加重启支持（更新包中包含启动依赖，需要重启后才能切换）
  使用方法：在打包过程中添加".reboot"文件即可，更新此版本更新包后会提示重启，重启后继续更新后续更新包

v2.2.3
添加停用通知（更新包无法解决的问题，必须手动导入更新包时使用）
  使用方法：在mcpath根目录添加.stop文件，显示的停用信息可以自定义（写入.stop文件）
